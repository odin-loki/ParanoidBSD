...
What is left, and what class it is:

| | |
|---|---|
| 47 | `curses.h` and friends — ncurses generates its own headers |
| 37 | not a missing header: `ipfilter`'s `ipsend`/`ipftest`/`ipresend`, which `sbin/ipf/Makefile:9` has **commented out** of `SUBDIR`, plus a handful of one-off compile errors |
| 13 | `type_traits` — C++ sources handed to a C analyser |
| 13 | `hostres_oid.h`, 6 `bridge_tree.h` — bsnmp modules, `gensnmptree` |
