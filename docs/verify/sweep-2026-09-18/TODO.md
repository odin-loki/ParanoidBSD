...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,788 rows, 6,292 PROVED, hole ~1,080. Unread new-FAILED is **0**. Current last is BOUNDED `GetStamp`; TIMEOUTs still filling (`hpt_do_tasks`). Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
