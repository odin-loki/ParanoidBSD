...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,879 rows, 6,294 PROVED, hole ~989. Unread new-FAILED is **0**. Three NTB FAILEDs (`amd_ntb_mw_{set,clear}_trans`, `intel_ntb_spad_write`) are unmodelled `device_get_softc`. Settled 65, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
