...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,759 rows, 6,292 PROVED, hole ~1,109. Unread new-FAILED is **0**. Current TIMEOUTs (`mtkswitch_reg_write`; live `smi_write`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
