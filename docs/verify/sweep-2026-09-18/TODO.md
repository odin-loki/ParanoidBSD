...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,873 rows, 6,294 PROVED, hole ~995. Unread new-FAILED is **0**. Current TIMEOUTs (`mvneta_tx_queue_enable`; live `nfe_miibus_writereg`). Settled 61, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
