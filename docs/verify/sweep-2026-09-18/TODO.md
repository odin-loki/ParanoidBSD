...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,779 rows, 6,292 PROVED, hole ~1,089. Unread new-FAILED is **0**. `bytgpio_pin_set` FAILED is unmodelled `device_get_softc` (not a bug). Current last row is that FAILED; workers still on-CPU. Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
