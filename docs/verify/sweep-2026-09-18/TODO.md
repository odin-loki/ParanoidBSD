...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,867 rows, 6,294 PROVED, hole ~1,001. Unread new-FAILED is **0**. Current TIMEOUTs (`mvs_edma` settled; live `mvneta_miibus_writereg`). Settled 61, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
