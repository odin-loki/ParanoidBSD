...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,727 rows, 6,292 PROVED, hole ~1,141. Unread new-FAILED is **0**. Current TIMEOUTs (`miiproxy_readreg`/`writereg`, `arswitch_atu_flush_port`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **584 OK / 39 ERROR / 2 TIMEOUT** of 625 flushed, 0 guessed. jsonl block-buffers (looked stuck at 585; clang++ was live on `solid` `udevmanager.cpp`). Findings 66: 56 Qt; kde-src set unchanged. Still not a KDE proof column.
