...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,893 rows, 6,294 PROVED, hole ~975. Unread new-FAILED is **0**. `cbb_power` FAILED is unmodelled `device_get_softc` / inlined `exca_getb` (not a bug). Current last is that FAILED. Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
