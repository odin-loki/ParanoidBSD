...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,885 rows, 6,294 PROVED, hole ~983. Unread new-FAILED is **0**. Current TIMEOUTs (`intel_ntb_mw_clear_trans`; live `ntb_plx_mw_set_trans`). Settled 66, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
