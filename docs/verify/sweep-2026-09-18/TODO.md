...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,801 rows, **6,293 PROVED** (+1 `ida_cmd_lookup`), hole ~1,067. Unread new-FAILED is **0**. Current last is PROVED `ida_cmd_lookup`; TIMEOUTs still filling (`hptrr hpt_shutdown_vbus`). Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
