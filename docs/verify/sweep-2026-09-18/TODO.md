...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,911 rows, 6,295 PROVED, hole ~957. Unread new-FAILED is **0**. Current last TIMEOUT `pci_set_powerstate_method`. Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
