...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,690 rows, 6,292 PROVED, hole ~1,178. Unread new-FAILED is **0**. Current TIMEOUTs (`bxe_debugnet_poll`, `cgem_miibus_*`) are unmodelled MMIO/softc. Settled 53, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **376 OK / 32 ERROR / 1 TIMEOUT** of 409 written, 0 guessed. ERROR set unchanged. Findings 35: 33 Qt NewDelete*; k7zip uninit `seqOutStream` (read); **`DBusHelperProxy.cpp:142` watcher leak is parented + `deleteLater`** — not-a-defect ([queue/kauth-dbus-watcher-leak.json](queue/kauth-dbus-watcher-leak.json)). Still not a KDE proof column.
