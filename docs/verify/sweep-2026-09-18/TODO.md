...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,865 rows, 6,294 PROVED, hole ~1,003. Unread new-FAILED is **0**. `mvs_edma` FAILED is unmodelled `device_get_softc` (not a bug). Settled 61, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
