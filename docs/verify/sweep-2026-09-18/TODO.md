...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,905 rows, 6,295 PROVED, hole ~963. Unread new-FAILED is **0**. Current TIMEOUTs (`pci_has_quirk` proved; live `pci_read_device`). Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
