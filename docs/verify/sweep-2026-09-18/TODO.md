...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,823 rows, **6,294 PROVED** (+1 `ipw_cmdname`), hole ~1,045. Unread new-FAILED is **0**. Current last is PROVED `ipw_cmdname`. Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
