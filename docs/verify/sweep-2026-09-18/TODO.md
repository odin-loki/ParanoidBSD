...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,733 rows, 6,292 PROVED, hole ~1,135. Unread new-FAILED is **0**. Current TIMEOUTs (`e6000sw_read_xmdio`/`write_xmdio`/`readphy`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
