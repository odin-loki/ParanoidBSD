...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,781 rows, 6,292 PROVED, hole ~1,087. Unread new-FAILED is **0**. Current TIMEOUTs (`bytgpio_pin_set` settled; live `gpio_pin_set_active`) are unmodelled MMIO/softc. Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
