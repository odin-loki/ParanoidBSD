...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,837 rows, 6,294 PROVED, hole ~1,031. Unread new-FAILED is **0**. Current TIMEOUTs (`mdio_readextreg`; live `mfi_dump_all`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
