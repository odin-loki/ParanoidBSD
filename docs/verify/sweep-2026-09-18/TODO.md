...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,856 rows, 6,294 PROVED, hole ~1,012. Unread new-FAILED is **0**. Current TIMEOUTs (`init_crc16tab` BOUNDED; live `mmcspi_send_stop`). Settled 60, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
