...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,798 rows, 6,292 PROVED, hole ~1,070. Unread new-FAILED is **0**. Current TIMEOUTs (`hptnr hpt_shutdown_vbus`; live `hptrr hpt_shutdown_vbus`). Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
