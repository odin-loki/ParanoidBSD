...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,851 rows, 6,294 PROVED, hole ~1,017. Unread new-FAILED is **0**. `ib_mask_to_mlx5_opt` is a type-level `1 << 31` defect (patched `1U`, third after gpioctl/makefs). `alloc_flow_table` FAILED is unmodelled `kzalloc`. Settled 60, **3 defects**. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
