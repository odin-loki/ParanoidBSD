...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,887 rows, 6,294 PROVED, hole ~981. Unread new-FAILED is **0**. `ntb_plx_mw_set_wc` FAILED is unmodelled `device_get_softc` (not a bug). Current last TIMEOUT `ntb_plx_mw_clear_trans`. Settled 67, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
