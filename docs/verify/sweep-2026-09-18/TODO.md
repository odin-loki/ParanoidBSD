...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,771 rows, 6,292 PROVED, hole ~1,097. Unread new-FAILED is **0**. Current TIMEOUTs (`ffec_miibus_readreg`; live `scmi_req_initialized_alloc`) are unmodelled MMIO/softc. Settled 56, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
