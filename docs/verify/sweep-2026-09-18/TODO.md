...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,900 rows, 6,294 PROVED, hole ~968. Unread new-FAILED is **0**. Current TIMEOUTs (`pci_hostb_read_config`; live `pci_hostb_write_config`). Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
