...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,790 rows, 6,292 PROVED, hole ~1,078. Unread new-FAILED is **0**. Current TIMEOUTs (`GetStamp` BOUNDED; live `fDeSetTCQ`). Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
