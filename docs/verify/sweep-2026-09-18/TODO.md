...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,767 rows, 6,292 PROVED, hole ~1,101. Unread new-FAILED is **0**. `vesa_bios_post` FAILED is unmodelled `devclass_get_devices` (not a bug). Current TIMEOUTs (`ukswitch_readphy`; live `ukswitch_writephy`) are unmodelled MMIO/softc. Settled 56, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
