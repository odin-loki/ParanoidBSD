...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,842 rows, 6,294 PROVED, hole ~1,026. Unread new-FAILED is **0**. Current last is BOUNDED `mii_fdt_lookup_phy`; TIMEOUTs still filling (`mgb_miibus_readreg`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
