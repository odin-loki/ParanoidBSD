...
8. **CBMC TIMEOUT resume (in flight).** Unwind-16 evidence is `cbmc-old.jsonl`. `--resume` only at unwind 32 / 180 s. Live file ~8,883 rows, hole ~1,985. Five new FAILED read, 0 defects. Five BOUNDED became PROVED. New TIMEOUT at 180 s is still TIMEOUT, not a bug. Do **not** pass `--retry-status` again.
...
12. **Done (19 Sep).** CodeQL CLI 2.27.0 is on PATH. `taxonomy.py --available` is 46/52 COVERED (0.88). Smokes: `abs.c` and `sys_ffclock_setestimate` (`copyin` of `ffclock_estimate`) are `SMOKE-OK`. That is not a tree taint run. Infer still absent; it buys no new COVERED class.
