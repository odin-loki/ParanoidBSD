...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,742 rows, 6,292 PROVED, hole ~1,126. Unread new-FAILED is **0**. Current TIMEOUTs (`e6060sw_writephy`; live `readreg`/`writereg`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
