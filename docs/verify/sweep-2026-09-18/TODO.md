...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,714 rows, 6,292 PROVED, hole ~1,154. Unread new-FAILED is **0**. Current TIMEOUTs (`dwc1000_miibus_*`, `if_dwc_rk_set_speed`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **469 OK / 39 ERROR / 2 TIMEOUT** of 510 written, 0 guessed. Seven new ERROR are kjobwidgets missing `moc_*.cpp` / generated dbus iface headers (stale autogen). `notifybypopup.cpp:359` watcher leak is `deleteLater` on `finished` — not-a-defect. Upstream defects remain k7zip + klocalimagecache only.
