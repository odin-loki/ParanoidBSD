...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,818 rows, 6,293 PROVED, hole ~1,050. Unread new-FAILED is **0**. Current TIMEOUTs (`i2c_repeated_start`; live `iicmux_callback`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
