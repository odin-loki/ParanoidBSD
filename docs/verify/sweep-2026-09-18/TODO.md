...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,785 rows, 6,292 PROVED, hole ~1,083. Unread new-FAILED is **0**. Current TIMEOUTs (`gpiobus_pin_set`; live `hpt_do_tasks`) are unmodelled MMIO/softc. Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
