...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,890 rows, 6,294 PROVED, hole ~978. Unread new-FAILED is **0**. Current TIMEOUTs (`ntb_plx_peer_spad_write`; live `oce_mbox_nic_set_iface_lro_config`). Settled 67, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
