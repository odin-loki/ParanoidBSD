...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,889 rows, 6,294 PROVED, hole ~979. Unread new-FAILED is **0**. Current TIMEOUTs (`ntb_plx_mw_clear_trans`; live `ntb_plx_peer_spad_write`). Settled 67, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
