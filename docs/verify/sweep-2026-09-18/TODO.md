...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,825 rows, 6,294 PROVED, hole ~1,043. Unread new-FAILED is **0**. Current TIMEOUTs (`ipw_cmdname` PROVED; live `irdma_prep_for_unregister`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
