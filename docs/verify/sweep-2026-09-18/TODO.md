...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,808 rows, 6,293 PROVED, hole ~1,060. Unread new-FAILED is **0**. Current TIMEOUTs (`iicbb_sendbit`; live `iicbb_start_impl`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
