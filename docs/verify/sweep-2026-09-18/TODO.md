...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,812 rows, 6,293 PROVED, hole ~1,056. Unread new-FAILED is **0**. Current TIMEOUTs (`iicbb_repstart`; live `iicoc_iicbus_start`). Previous iicbb balloon timed out without killing the driver; one live worker still ~24 GiB RSS. Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
