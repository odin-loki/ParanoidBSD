...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,680 rows, 6,292 PROVED, hole ~1,188. Unread new-FAILED is **0**. Next TIMEOUTs (`axgbe_if_promisc_set`, `bce_miibus_read_reg`) are unmodelled MMIO/softc. Settled 53, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **332 OK / 32 ERROR / 1 TIMEOUT** of 365 written, 0 guessed. ERROR is DB-honest: kwindowsystem never got `QtGuiPrivate` `-I` (header is installed), stale autogen/moc, kjobwidgets paths gone, two Solid `.c` TUs `-std=c99` under clang++. TIMEOUT is `kwayland/.../registry.cpp`. Findings: 30 Qt NewDelete*; **1 read `karchive` `k7zip.cpp:1803` uninitialized `seqOutStream`** (KF 6.18 unpack under `kde-src`, not vendored here — [queue/k7zip-uninit-seqOutStream.json](queue/k7zip-uninit-seqOutStream.json)). Still not a KDE proof column.
