...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,810 rows, 6,293 PROVED, hole ~1,058. Unread new-FAILED is **0**. Current TIMEOUTs (`iicbb_start_impl`; live `iicbb_repstart`). RSS pressure: ~45 GiB used / 2 GiB avail on the two `iicbb` workers. Do not SIGTERM. Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
