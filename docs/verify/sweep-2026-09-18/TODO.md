...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,844 rows, 6,294 PROVED, hole ~1,024. Unread new-FAILED is **0**. Current TIMEOUTs (`mii_fdt_lookup_phy` BOUNDED; live `mlx4_en_change_mtu`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
