...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,903 rows, 6,295 PROVED, hole ~965. Unread new-FAILED is **0**. Hole-fill PROVED `pci_has_quirk`. Current last is that PROVED. Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
