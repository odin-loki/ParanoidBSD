...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,883 rows, 6,294 PROVED, hole ~985. Unread new-FAILED is **0**. `intel_ntb_mw_set_wc` FAILED is unmodelled `device_get_softc` (not a bug). Current last TIMEOUT `intel_ntb_mw_clear_trans`. Settled 66, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
