...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,881 rows, 6,294 PROVED, hole ~987. Unread new-FAILED is **0**. Current TIMEOUTs (NTB FAILEDs settled; live `intel_ntb_mw_set_trans`). Settled 65, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
