...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,739 rows, 6,292 PROVED, hole ~1,129. Unread new-FAILED is **0**. Current TIMEOUTs (`e6000sw_readreg_wrapper`/`writereg_wrapper`, `e6060sw_dot1q_mode`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
