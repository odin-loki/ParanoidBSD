...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,863 rows, 6,294 PROVED, hole ~1,005. Unread new-FAILED is **0**. Current TIMEOUTs (`msk_miibus_writereg`; live `mvs_legacy_intr`). Settled 60, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
