...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,757 rows, 6,292 PROVED, hole ~1,111. Unread new-FAILED is **0**. Current TIMEOUTs (`ksz8995ma_writereg`; live `mtkswitch_reg_write`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
