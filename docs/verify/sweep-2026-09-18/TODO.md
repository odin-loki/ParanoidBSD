...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,737 rows, 6,292 PROVED, hole ~1,131. Unread new-FAILED is **0**. Current TIMEOUTs (`e6000sw_writephy`/`writephy_locked`/`readreg_wrapper`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
