...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,748 rows, 6,292 PROVED, hole ~1,120. Unread new-FAILED is **0**. Current TIMEOUTs (`felix_readreg_wrapper`; live `adm6996fc_readphy`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
