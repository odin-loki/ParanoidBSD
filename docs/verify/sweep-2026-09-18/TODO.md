...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,806 rows, 6,293 PROVED, hole ~1,062. Unread new-FAILED is **0**. Current TIMEOUTs (`igc_if_msix_intr_assign`; live `iicbb_sendbit`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
