...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,829 rows, 6,294 PROVED, hole ~1,039. Unread new-FAILED is **0**. Current TIMEOUTs (`irdma_ceq_tasklet_cb`; live `ismt_quick`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
