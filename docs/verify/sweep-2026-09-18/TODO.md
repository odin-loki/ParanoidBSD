...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,783 rows, 6,292 PROVED, hole ~1,085. Unread new-FAILED is **0**. Current TIMEOUTs (`gpio_pin_set_active`; live `gpiobus_pin_set`) are unmodelled MMIO/softc. Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
