...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,651 rows, 6,291 PROVED, hole ~1,217. Two more TIMEOUT→FAILED (`ata_jmicron_setmode`, `ata_marvell_setmode`) are unmodelled `device_get_softc`. Settled 52, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (19 Sep).** 726-TU `--compile-commands-only` ingest in flight (jobs=1): 35/726 OK so far, 0 guessed, 0 ERROR. One finding is clang `cplusplus.NewDelete` inside Qt `qsharedpointer_impl.h`, not a KDE source file. Still not a KDE proof column.
