...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,777 rows, 6,292 PROVED, hole ~1,091. Unread new-FAILED is **0**. Current TIMEOUTs (`fxp_miibus_writereg`; live `gem_mii_readreg`) are unmodelled MMIO/softc. Settled 56, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
