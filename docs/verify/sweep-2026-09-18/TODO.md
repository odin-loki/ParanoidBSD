...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,738 rows, 6,292 PROVED, hole ~1,130. Unread new-FAILED is **0**. Current TIMEOUTs (`e6000sw_writephy_locked`/`readreg_wrapper`/`writereg_wrapper`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
