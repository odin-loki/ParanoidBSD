...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,731 rows, 6,292 PROVED, hole ~1,137. Unread new-FAILED is **0**. Current TIMEOUTs (`arswitch_writephy`, `e6000sw_serdes_power`/`read_xmdio`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **626 OK / 39 ERROR / 2 TIMEOUT** of 667 written, 0 guessed. Findings 68: 58 Qt; kde-src set unchanged. Still not a KDE proof column.
