...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,775 rows, 6,292 PROVED, hole ~1,093. Unread new-FAILED is **0**. Current TIMEOUTs (`scmi_msg_get`; live `fxp_miibus_writereg`) are unmodelled MMIO/softc. Settled 56, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
