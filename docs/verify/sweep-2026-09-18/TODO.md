...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,804 rows, 6,293 PROVED, hole ~1,064. Unread new-FAILED is **0**. Current TIMEOUTs (`igc_if_set_promisc`; live `igc_if_msix_intr_assign`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
