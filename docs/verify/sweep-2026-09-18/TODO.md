...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,794 rows, 6,292 PROVED, hole ~1,074. Unread new-FAILED is **0**. Current TIMEOUTs (`fDeSetWriteCache`; live `fOsBuildSgl`). Settled 57, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
