...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,827 rows, 6,294 PROVED, hole ~1,041. Unread new-FAILED is **0**. Current TIMEOUTs (`irdma_prep_for_unregister`; live `irdma_ceq_tasklet_cb`). RSS ~32 GiB used / 14 GiB avail. Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
