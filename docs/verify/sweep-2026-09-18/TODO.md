...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,741 rows, 6,292 PROVED, hole ~1,127. Unread new-FAILED is **0**. Current TIMEOUTs (`e6060sw_set_vtu`/`readphy`; live `writephy`/`readreg`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
