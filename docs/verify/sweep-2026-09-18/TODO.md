...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,895 rows, 6,294 PROVED, hole ~973. Unread new-FAILED is **0**. Current TIMEOUTs (`cbb_power`; live `cbb_read_config`). Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
