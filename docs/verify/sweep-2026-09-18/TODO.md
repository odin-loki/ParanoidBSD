...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,763 rows, 6,292 PROVED, hole ~1,105. Unread new-FAILED is **0**. Current TIMEOUTs (`rtl_readreg`; live `rtl_readphy`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
