...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,723 rows, 6,292 PROVED, hole ~1,145. Unread new-FAILED is **0**. Current TIMEOUTs (`eqos_miibus_*`, `et_miibus_readreg`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **544 OK / 39 ERROR / 2 TIMEOUT** of 585 written, 0 guessed. Findings 62: 52 Qt; kde-src set unchanged (k7zip + klocalimagecache still the only read upstream defects). Still not a KDE proof column.
