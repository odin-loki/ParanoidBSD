...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,897 rows, 6,294 PROVED, hole ~971. Unread new-FAILED is **0**. Current TIMEOUTs (`cbb_read_config`; live `pcf_repeated_start`). Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
