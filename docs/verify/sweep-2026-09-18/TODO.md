...
8. **CBMC TIMEOUT resume (OOM, restarting).** Overnight resume died: OOM-killed a `cbmc` worker (~19 GB) at `[800/2048]`. Live file **9,640** rows, **6,291 PROVED**, hole ~**1,228**. Fourteen overnight TIMEOUT/BOUNDED→FAILED read: **0 defects** (`1 << queue` is 0..4; rest unmodelled `device_get_softc` / OpenZFS contrib). Restart `--resume` only at **JOBS=2** so it does not OOM again. Do **not** pass `--retry-status`.
