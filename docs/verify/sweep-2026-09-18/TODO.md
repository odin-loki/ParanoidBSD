...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,876 rows, 6,294 PROVED, hole ~992. Unread new-FAILED is **0**. `nge_miibus_readreg` FAILED is unmodelled `device_get_softc` (not a bug). Current last TIMEOUT `nfsmb_writew`. Settled 62, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
