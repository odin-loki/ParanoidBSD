...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,869 rows, 6,294 PROVED, hole ~999. Unread new-FAILED is **0**. Current TIMEOUTs (`mvneta_miibus_writereg`; live `mvneta_tx_queue_init`). Settled 61, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
