...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,820 rows, 6,293 PROVED, hole ~1,048. Unread new-FAILED is **0**. Current TIMEOUTs (`iicmux_callback`; live `imcsmb_writew`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
