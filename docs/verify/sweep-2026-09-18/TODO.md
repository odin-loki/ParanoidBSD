...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,831 rows, 6,294 PROVED, hole ~1,037. Unread new-FAILED is **0**. Current TIMEOUTs (`ismt_quick`; live `jme_miibus_readreg`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
