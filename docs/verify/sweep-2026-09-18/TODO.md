...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,816 rows, 6,293 PROVED, hole ~1,052. Unread new-FAILED is **0**. Current TIMEOUTs (`twsi_repeated_start`; live `i2c_repeated_start`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
