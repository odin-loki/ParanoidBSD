...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,861 rows, 6,294 PROVED, hole ~1,007. Unread new-FAILED is **0**. Current TIMEOUTs (`mmcspi_update_crc_setting`; live `msk_miibus_writereg`). mmcspi balloon timed out without killing the driver; RSS back to ~5 GiB. Settled 60, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
