...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,803 rows, 6,293 PROVED, hole ~1,065. Unread new-FAILED is **0**. `ichsmb_writew` FAILED is unmodelled `device_get_softc` (not a bug). Current last TIMEOUT `igc_if_set_promisc`. Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
