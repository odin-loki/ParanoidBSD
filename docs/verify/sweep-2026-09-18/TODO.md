...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,642 rows, 6,291 PROVED, hole ~1,226. Overnight-new FAILED already settled (50). Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (19 Sep).** 148/149 was the old ingest. A 726-TU `--compile-commands-only` run is in flight (jobs=1). `--compile-commands-only` now uses the database as the universe so a cmake `--root` is not NOT RUN. Still not a KDE proof column. Evidence: [queue/cxx-analyze-ccdb-726-digest.json](queue/cxx-analyze-ccdb-726-digest.json).
