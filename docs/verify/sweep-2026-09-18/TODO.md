...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,753 rows, 6,292 PROVED, hole ~1,115. Unread new-FAILED is **0**. Current TIMEOUTs (`adm6996fc_writereg`; live `ksz8995ma_writephy`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
