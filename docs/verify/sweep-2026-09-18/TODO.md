...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,845 rows, 6,294 PROVED, hole ~1,023. Unread new-FAILED is **0**. Current TIMEOUTs (`mlx4_en_change_mtu`; live `mlx4_en_set_ring_size`). Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
