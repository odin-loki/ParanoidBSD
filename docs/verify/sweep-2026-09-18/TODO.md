...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,814 rows, 6,293 PROVED, hole ~1,054. Unread new-FAILED is **0**. Current TIMEOUTs (`iicoc_iicbus_start`; live `twsi_repeated_start`). RSS back down to ~5 GiB used. Settled 58, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
