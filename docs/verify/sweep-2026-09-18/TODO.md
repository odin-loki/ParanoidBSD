...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,761 rows, 6,292 PROVED, hole ~1,107. Unread new-FAILED is **0**. Current TIMEOUTs (`smi_write`; live `rtl_readreg`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
