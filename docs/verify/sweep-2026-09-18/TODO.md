...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,699 rows, 6,292 PROVED, hole ~1,169. Unread new-FAILED is **0**. Current TIMEOUTs (`rk_cru_reset_assert`, `zynqmp_reset_assert`, `cxgb_ioctl`) are unmodelled MMIO/softc. Settled 53, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **404 OK / 32 ERROR / 1 TIMEOUT** of 437 written, 0 guessed. Findings 44: 36 Qt; k7zip uninit `seqOutStream` still the only read kde-src defect; six new kcompletion/kdbusaddons hits are clang FP / parented `deleteLater` / `free_since` precondition ([queue/kde-src-clang-findings.json](queue/kde-src-clang-findings.json)). Still not a KDE proof column.
