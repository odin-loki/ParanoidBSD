...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,859 rows, 6,294 PROVED, hole ~1,009. Unread new-FAILED is **0**. Current TIMEOUTs (`mmcspi_send_stop`; live `mmcspi_update_crc_setting`). RSS pressure: ~43 GiB used / 3.6 GiB avail. Do not SIGTERM. Settled 60, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
