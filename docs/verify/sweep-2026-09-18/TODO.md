...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,735 rows, 6,292 PROVED, hole ~1,133. Unread new-FAILED is **0**. Current TIMEOUTs (`e6000sw_readphy`/`readphy_locked`/`writephy`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
