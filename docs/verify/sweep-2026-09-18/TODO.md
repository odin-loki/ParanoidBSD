...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,676 rows, 6,292 PROVED, hole ~1,192. Unread new-FAILED is **0** after settling `ata_via_sata_scr_write` (unmodelled `device_get_parent` / kobj ops). Settled 53, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU `--compile-commands-only` ingest in flight (jobs=1, pid 4980): **261 OK / 23 ERROR / 1 TIMEOUT** of 285 written, 0 guessed. The 23 ERROR are missing Qt private `qtx11extras_p.h`, missing `moc_*.cpp`, or kjobwidgets build-dir paths that are gone — not KDE source defects. Findings so far are clang on Qt headers (`qsharedpointer_impl.h` NewDelete, `qobjectdefs.h` NewDeleteLeaks). Still not a KDE proof column.
