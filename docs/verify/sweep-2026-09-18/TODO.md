...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,708 rows, 6,292 PROVED, hole ~1,160. Two more TIMEOUT→FAILED (`cxgb_debugnet_poll`, `dc_miibus_writereg`) are unmodelled `if_getsoftc` / `device_get_softc`. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.

## Cover the trees this run left UNTOUCHED

9. **KDE compile DB (20 Sep).** 726-TU ingest in flight (pid 4980): **443 OK / 32 ERROR / 2 TIMEOUT** of 477 written, 0 guessed. Findings 48: 39 Qt; k7zip uninit `seqOutStream` and **`klocalimagecacheimpl.cpp:90` `new QPixmap` dropped when caching is off / pixmap is null** (upstream, not vendored — [queue/klocalimagecache-pixmap-leak.json](queue/klocalimagecache-pixmap-leak.json)). Still not a KDE proof column.
