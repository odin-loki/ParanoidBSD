...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,899 rows, 6,294 PROVED, hole ~969. Unread new-FAILED is **0**. Current TIMEOUTs (`pcf_repeated_start`; live `pci_hostb_read_config`). Settled 68, 3 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
