...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,765 rows, 6,292 PROVED, hole ~1,103. Unread new-FAILED is **0**. Current TIMEOUTs (`rtl_readphy`; live `ukswitch_readphy`) are unmodelled MMIO/softc. Settled 55, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
