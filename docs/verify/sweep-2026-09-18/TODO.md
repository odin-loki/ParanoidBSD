...
8. **CBMC TIMEOUT resume (JOBS=2).** Live file 9,769 rows, 6,292 PROVED, hole ~1,099. Unread new-FAILED is **0**. Current TIMEOUTs (`ukswitch_writephy`; live `ffec_miibus_readreg`) are unmodelled MMIO/softc. Settled 56, still 2 defects. Pid 887 `--resume` only. Do **not** pass `--retry-status`.
