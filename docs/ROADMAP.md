...
`docs/PORT_PLAN.md` is the same work at the other end of the telescope: one
numbered, tagged entry per file PBSD owns and per function in it — 23,918
files and 114,217 functions, 26.2 million lines accounted for. This document
says what to do; that one is what you pick an item out of. It is generated by
`tools/port_plan.py`, so re-run it and commit the diff rather than editing it.
