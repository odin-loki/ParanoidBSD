...
EXPECTED = {
    ...
    # The one translation unit crypto/openssh's shard costs, and it is
    # worth saying exactly what it costs it for. The tree carries TWO
    # GSS-API implementations whose public headers collide:
    # include/gssapi/Makefile installs FreeBSD's gssapi/gssapi.h and
    # krb5/lib/gssapi/generic/Makefile.inc:35-38 installs MIT's into the
    # same place, neither guarded by MK_MITKRB5, so which one an
    # installed tree holds is decided by the order Makefile.inc1's
    # SUBDIR reaches them. incs_shim() cannot reproduce that, because
    # it builds the layout out of the headers that exist AS FILES:
    # gssapi/gssapi_ext.h is a plain file, so the shim has MIT's, and
    # gssapi/gssapi.h is GENERATED by that Makefile.inc's shell rule, so
    # the shim has none and the name resolves to FreeBSD's. That pair is
    # one no installed tree ever holds.
    #
    # mitkrb5_shim() exists for the two directories that state which one
    # they mean -- usr.sbin/gssd and lib/libpam/modules/pam_ksu, the
    # only two in the tree that write `CFLAGS+= -DMK_MITKRB5=yes'. This
    # file's component does not. Widening the predicate to the LIBADD
    # that names MIT's library was tried and measured: it fixes this
    # file and breaks lib/librpcsec_gss/svc_rpcsec_gss.c, whose Makefile
    # has the identical LIBADD shape and which calls gss_pname_to_uid(),
    # declared at include/gssapi/gssapi.h:864 and absent from MIT's
    # gssapi.hin. Which library a component LINKS is not which header
    # its sources are compiled AGAINST.
    "crypto/openssh/gss-serv-krb5.c":
        "wants MIT's <gssapi/gssapi.h>, which is generated and so is "
        "not in the installed-header shim; MIT's gssapi_ext.h IS, "
        "because it is a plain file, and its gss_const_buffer_t at "
        ":517 is a type only MIT's gssapi.h defines. The file is named "
        "by the build -- secure/libexec/sshd-auth and sshd-session -- "
        "so it is here by name rather than under a prefix",

    # cddl/, opened to the analyser after it had been excused from every
    # shard as "third-party source PBSD does not maintain". It ships
    # dtrace(1), libdtrace, libctf and the ZFS userland tools; 127 of
    # its 135 translation units compile and the eight below are the
    # whole remainder. Not one of them is in the shipped binaries.
    "cddl/contrib/opensolaris/cmd/dtrace/test/cmd/badioctl/badioctl.c":
        "a hand-run probe that hammers ioctl(2) at the dtrace device "
        "while a second process traces it. No Makefile in the tree "
        "names it - the test suite's Makefiles list the .d scripts and "
        "the CFILES under test/tst, never test/cmd. It also calls "
        "kill(2) with no <signal.h>, which is why it is visible at "
        "all. NOT_NAMED",
    "cddl/contrib/opensolaris/lib/pyzfs/common/ioctl.c":
        "the Python bindings to libzfs_core. FreeBSD builds no part of "
        "pyzfs - the only Makefile in the tree that mentions it is "
        "openzfs's own autotools Makefile.am, which this build does not "
        "run - and it wants <Python.h>. NOT_NAMED",

    # The four the test suite's own Makefile excludes by name. Its
    # first two lines are the whole reason:
    #
    #   # We exclude several subdirectories: nfs and sysevent do not
    #   # compile on FreeBSD, and docsExamples doesn't seem amenable
    #   # to automated testing.
    #
    # bmake IN cddl/usr.sbin/dtrace/tests/common/nfs does name
    # tst.call.c, so NOT_NAMED is false here; what is true is that
    # nothing descends into that directory. It is not spelled SUBDIR
    # either - bsd.test.mk's word is TESTS_SUBDIRS - so it needs its
    # own claim, which test_expected_errors.py checks in both halves:
    # the build directory really does name the file, and its parent
    # really does leave it out.
    "cddl/contrib/opensolaris/cmd/dtrace/test/tst/common/nfs/tst.call.c":
        "wants <rpcsvc/nfs4_prot.h>, which is Solaris's rpcgen output "
        "for a protocol description this tree does not carry. "
        "NOT_TESTS_SUBDIR",
    "cddl/contrib/opensolaris/cmd/dtrace/test/tst/common/nfs/tst.call3.c":
        "the same missing rpcsvc header, plus the NFSv3 XDR routines "
        "its dispatch table names. NOT_TESTS_SUBDIR",
    "cddl/contrib/opensolaris/cmd/dtrace/test/tst/common/sysevent/"
    "tst.post.c":
        "wants <libsysevent.h>. libsysevent is Solaris's system event "
        "framework; FreeBSD has never had it, and nothing in the tree "
        "provides the header. NOT_TESTS_SUBDIR",
    "cddl/contrib/opensolaris/cmd/dtrace/test/tst/common/sysevent/"
    "tst.post_chan.c":
        "the same missing <libsysevent.h>, for the event-channel half "
        "of the same test. NOT_TESTS_SUBDIR",

    # ...and the two that ARE built, and fail on a header dtrace(1)
    # writes. dtrace.test.mk:33 adds the provider definition to the
    # program's SRCS -
    #
    #   .if exists(${prog:S/^tst.//:S/.exe$/.d/})
    #   SRCS.${prog}+= ${prog:S/^tst.//:S/.exe$/.d/}
    #
    # - and bsd.prog.mk's .d.h rule runs `dtrace -h -s' over it. The
    # emitter is dtrace's own; there is no second implementation of it
    # to run here, and writing one would be inventing the answer rather
    # than reading it.
    "cddl/contrib/opensolaris/cmd/dtrace/test/tst/common/json/tst.usdt.c":
        "usdt.h is generated by `dtrace -h -s usdt.d', which needs a "
        "built dtrace(1). A generated header, the way device_if.h is",
    "cddl/contrib/opensolaris/cmd/dtrace/test/tst/common/usdt/"
    "tst.forker.c":
        "forker.h is generated by `dtrace -h -s forker.d', the same "
        "way, from the provider definition beside it",
}
...
NOT_BUILT = {
    ...
    # OpenSSH's portability layer and its test corpus. crypto/ was
    # excused from every shard as "third-party source PBSD does not
    # maintain", and crypto/openssh is the ssh client and the sshd this
    # system ships. 181 of its 297 translation units compile; every one
    # of the other 116 is named by nothing in the build, which
    # sweep_report.names_it() is asked about each of them one at a time
    # -- see not_built() above for why the condition is the claim.
    "crypto/openssh/openbsd-compat/":
        "OpenSSH's portability layer. secure/lib/libssh/Makefile:36-60 "
        "names the 18 of these FreeBSD does not already provide; the "
        "rest are replacements for arc4random(3), basename(3), "
        "strlcpy(3) and their neighbours, which this system has. What "
        "an unbuilt one asks for is <includes.h>, and no Makefile "
        "supplies the -I because no Makefile compiles it. "
        "NOT_NAMED_UNDER",
    "crypto/openssh/regress/":
        "the upstream regression suite. FreeBSD runs the shell scripts "
        "through secure/usr.bin/ssh/tests and compiles none of the C: "
        "the unit-test drivers, the fuzz harnesses and sk-dummy all "
        "want <includes.h> or a crypto_api.h the build does not lay "
        "out, and the build names not one of them. NOT_NAMED",
    "crypto/openssh/contrib/":
        "the askpass helpers upstream ships for other desktops. They "
        "want <X11/Xlib.h> and <gnome.h>; FreeBSD builds "
        "x11-ssh-askpass from ports instead, and nothing here names "
        "them. NOT_NAMED",
...
def _build_names_it(rel: str) -> bool:
    """sweep_report.names_it(), imported late.

    sweep_report imports THIS module, so the import cannot be at the
    top. By the time a prefix is resolved both modules are loaded.
    """
    import sweep_report
    return sweep_report.names_it(rel)
...
def not_built(rel: str) -> str | None:
    ...
    excuses a file under it only while the build names nothing by that
    name, and a file the build does name falls straight through to the
    per-file table. That is strictly safer than a plain prefix, which
    excuses everything beneath it - and it is the only shape that fits
    a vendor directory the build picks through.

    crypto/openssh/openbsd-compat is the case. It holds OpenSSH's
    portability layer: 88 sources, of which secure/lib/libssh's SRCS
    names 18 - the ones FreeBSD's libc does not already provide - and
    the other 70 are replacements for arc4random(3), basename(3),
    strlcpy(3) and so on that this system has. Those 70 are compiled by
    nothing and reach for an <includes.h> no -I can supply, because no
    Makefile supplies one. A plain prefix over that directory would
    swallow the 18 that ARE built, which is exactly the hole this file
    exists to prevent; 70 individual entries would say the same
    sentence seventy times and go stale the moment upstream moves a
    file from one side of the line to the other. The condition is the
    claim, so it cannot go stale: it is recomputed from the build.
    """
    for pre, why in NOT_BUILT.items():
        if not rel.startswith(pre):
            continue
        if why.endswith("NOT_NAMED_UNDER") and _build_names_it(rel):
            continue
        return pre
    return None
