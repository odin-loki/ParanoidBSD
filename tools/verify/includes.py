...
# The top-level directories whose sources _component_dir() will look for
# a building Makefile under. A source outside this set gets no CFLAGS
# from bmake at all -- only rpc_headers(), incs_shim(), the generated
# shim and its own directory, which is about seven -I against the twenty
# the build passes.
#
# contrib and crypto were outside it, and that is most of why 9.6% of
# crypto and 11.6% of contrib compiled. Both are reached the same way
# the rest of userland is: a Makefile somewhere else names the source
# through a .PATH -- crypto/openssh/ssh.c from secure/usr.bin/ssh,
# contrib/tcpdump/print-ip.c from usr.sbin/tcpdump -- and
# userland_names.builders() already resolves exactly that. It was
# gated out, not missing.
USERLAND_TOP = ("lib", "libexec", "bin", "sbin", "usr.bin",
                "usr.sbin", "games", "secure", "cddl",
                "contrib", "crypto")
...
def _gen_csh_iconv(out: Path, _dir: str = "") -> None:
    """bin/csh object-directory headers: iconv.h and GENHDRS.
        ...
        sh.err.h: sh.err.c
            grep 'ERR_' ${.ALLSRC} | grep '^#define'
        ed.defns.h: ed.defns.c
            grep '[FV]_' ${TCSHDIR}/ed.defns.c | grep '^#define'
        tc.const.h:
            ${CC} -E ${CFLAGS} ... -D_h_tc_const | grep 'Char STR' | sed ...
    ...
    The three grep/cc -E headers are the same shape as device_if.h:
    the Makefile names the recipe, the inputs are in the tree, and
    51 contrib/tcsh translation units failed on sh.err.h while this
    shim wrote only iconv.h. tc.defs.c is a .c (gethost), not a header.
    """
    src = SRC / "bin" / "csh" / "iconv_stub.h"
    if src.is_file():
        shutil.copyfile(src, out / "iconv.h")

    tcsh = SRC / "contrib" / "tcsh"
    errc = tcsh / "sh.err.c"
    if errc.is_file():
        lines = ["/* Do not edit this file, make creates it. */",
                 "#ifndef _h_sh_err", "#define _h_sh_err"]
        for ln in errc.read_text(errors="replace").splitlines():
            if ln.startswith("#define") and "ERR_" in ln:
                lines.append(ln)
        lines.append("#endif /* _h_sh_err */")
        (out / "sh.err.h").write_text("\n".join(lines) + "\n")

    edc = tcsh / "ed.defns.c"
    if edc.is_file():
        lines = ["/* Do not edit this file, make creates it. */",
                 "#ifndef _h_ed_defns", "#define _h_ed_defns"]
        for ln in edc.read_text(errors="replace").splitlines():
            if ln.startswith("#define") and ("F_" in ln or "V_" in ln):
                lines.append(ln)
        lines.append("#endif /* _h_ed_defns */")
        (out / "ed.defns.h").write_text("\n".join(lines) + "\n")

    # Makefile strips HAVE_ICONV from CFLAGS and predefines _h_tc_const
    # so tc.h does not include the header being written. Host cc -E is
    # what the recipe names (${CC}); a failure leaves the header absent.
    tcc = tcsh / "tc.const.c"
    if tcc.is_file():
        cshdir = SRC / "bin" / "csh"
        try:
            r = subprocess.run(
                ["cc", "-E", f"-I{out}", f"-I{cshdir}", f"-I{tcsh}",
                 "-D_h_tc_const", str(tcc)],
                capture_output=True, text=True, timeout=60)
            ...
            for ln in r.stdout.splitlines():
                if "Char STR" in ln:
                    ext.append(re.sub(
                        ...
                        r"extern Char \1[];", ln))
            if ext:
                body = ["/* Do not edit this file, make creates it. */",
                        "#ifndef _h_tc_const", "#define _h_tc_const"]
                body.extend(sorted(set(ext)))
                body.append("#endif /* _h_tc_const */")
                (out / "tc.const.h").write_text("\n".join(body) + "\n")
...
def include_flags(src: Path, arch: str = "amd64", cc: str = "clang",
    ...
    # The predicate is the -D and NOT the component's LIBADD, and that
    # was measured rather than assumed. `LIBADD+= gssapi_krb5' under
    # `.if ${MK_MITKRB5} == "no" / .else' looks like the same statement
    # -- secure/usr.bin/ssh, secure/usr.sbin/sshd and the two
    # secure/libexec/sshd-* all write it that way and none of them
    # spells the option as a -D -- and adding it to this test does fix
    # crypto/openssh/gss-serv-krb5.c. Over the 4,894 translation units
    # in sys/kern, sys/netinet, sys/dev/usb, lib, bin, sbin, usr.bin,
    # usr.sbin, stand, secure, kerberos5 and share it also BREAKS one:
    # lib/librpcsec_gss/svc_rpcsec_gss.c, which reaches
    # <gssapi/gssapi.h> through <rpc/rpcsec_gss.h> and calls
    # gss_pname_to_uid(). include/gssapi/gssapi.h declares that at :864;
    # MIT's gssapi.hin does not, because MIT puts it in gssapi_ext.h.
    # lib/librpcsec_gss/Makefile:9-13 is the same LIBADD shape.
    #
    # So which LIBRARY a component links is not which HEADER its
    # sources are compiled against, and the two only coincide for some
    # of the components that write both. The -D is narrower and it is
    # what the two directories that really want MIT's header actually
    # say. gss-serv-krb5.c is recorded in expected_errors.py instead,
    # with the reason, which is a shim this tree cannot make consistent:
    # `gssapi/gssapi_ext.h' is a plain file so it is MIT's, and
    # `gssapi/gssapi.h' is GENERATED by krb5/lib/gssapi/generic so it is
    # not in the shim at all and resolves to FreeBSD's. No installed
    # tree ever holds that pair; the shim does, because it can only
    # carry the headers that exist as files.
