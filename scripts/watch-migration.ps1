# Live PBSD migration console — watch progress without stopping the driver.
...
while ($true) {
    try {
        ...
        $status = wsl -d Ubuntu -- bash -lc 'cd ~/pbsd && python3 pbsd.py --status 2>/dev/null | head -14'
...
printf "agents:   "; pgrep -cf cursor-agent 2>/dev/null || echo 0
