...
static void	ref_login_protect(const char *, char *, int, uid_t, gid_t);
...
void
ref_login_fbtab(char *tty, uid_t uid, gid_t gid)
{
    FILE   *fp;
    char    buf[BUFSIZ];
    char   *devname;
	...
	if (strcmp(devname + 5, tty) == 0) {
		...
		ref_login_protect(table, cp, prot, uid, gid);
	    }
	}
    }
    fclose(fp);
...
static void
ref_login_protect(const char *table, char *pattern, int mask, uid_t uid, gid_t gid)
{
    glob_t  gl;
    char   *path;
    unsigned int     i;
