...
static void ref_write_sample_client(const char *, version_list * );
static void ref_write_sample_server( definition * );
static void ref_return_type( proc_list * );

void
ref_write_sample_svc(definition *def)
{

	if (def->def_kind != DEF_PROGRAM)
	  return;
	ref_write_sample_server(def);
}


int
ref_write_sample_clnt(definition *def)
{
        version_list *vp;
	int count = 0;
	...
	for (vp = def->def.pr.versions; vp != NULL; vp = vp->next) {
	  ref_write_sample_client(def->def_name, vp);
	  ++count;
	}
	return(count);
}
...
static void
ref_write_sample_client(const char *program_name, version_list *vp)
{
	proc_list *proc;
	int i;
	decl_list *l;
...
static void
ref_write_sample_server(definition *def)
{
	version_list *vp;
	proc_list *proc;
...
static void
ref_return_type(proc_list *plist)
{
  ptype(plist->res_prefix, plist->res_type, 1);
}

void
ref_add_sample_msg(void)
{
	f_print(fout, "/*\n");
...
void
ref_write_sample_clnt_main(void)
{
	list *l;
	definition *def;
	version_list *vp;
