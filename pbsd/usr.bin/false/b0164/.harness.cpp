/*
 * harness.cpp -- differential test for PBSD batch b0164 (false.c).
 *
 * false.c defines only main(void), which always returns 1.  Each case calls
...
import pbsd.usr.bin.f‍alse.b0164;
...
static Rng rng(0x00b0164faceULL);
...
test_main_hand()
{
	...
	run_case("repeat_3", 1);
	run_case("not_zero", 1);
	run_case("not_neg_one", 1);
	run_case("not_two", 1);
	run_case("exact_one", 1);
	run_case("boundary_below", 1);
	run_case("boundary_above", 1);
}
...
main()
{
	std::printf("PBSD batch b0164 differential test\n");
