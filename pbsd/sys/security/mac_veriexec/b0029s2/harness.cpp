/*
 * harness.cpp -- differential test for PBSD batch b0029s2.
 *
 * Batch b0029s2 consists of the single source file
 * hbsd/src/sys/security/mac_veriexec/mac_veriexec_sha256.c, which defines no
 * functions: its entire body is the kernel-only MAC_VERIEXEC_FPMOD()
 * registration macro.  That macro was recorded in skipped.txt instead of
 * being ported, so port.cppm exports no functions and oracle.c declares no
 * ref_ functions.
 *
...
import pbsd.sys.security.mac.veriexec.b0029s2;
...
main(void)
{
	std::printf("PBSD batch b0029s2 differential test\n");
	...
	std::printf("mac_veriexec_sha256.c defines no functions; see "
	    "skipped.txt.\n");
