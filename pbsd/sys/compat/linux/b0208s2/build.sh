#!/bin/sh
#
# build.sh -- build and run the PBSD b0208s2 differential test.
#
# Usage: sh build.sh            (from pbsd/sys/compat/linux/b0208s2/)
#
...
BUILD=build
MODNAME=pbsd.sys.compat.linux.b0208s2
...
$CXX $CXXFLAGS $MODFLAGS "$BUILD/port.o" "$BUILD/harness.o" \
    "$BUILD/oracle.o" -o "$BUILD/b0208s2_test"

exec "$BUILD/b0208s2_test"
