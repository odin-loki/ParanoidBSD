 ...
 * unmodified C oracle.
 *
 * sinpil() is driven with hand written edge cases and a fixed-seed randomised
...
/*
 * Every threshold sinpil tests against, both sides of each one, every
 * quarter-integer (argument reduction keys on 0, 0.25, 0.5 and 0.75 exactly),
 * the 0x1p112 huge-argument threshold, the 0x1p-60 tiny-argument threshold and
...
main(void)
{
	...
	for (i = 0; i < NEDGE; i++)
		case_sinpil(edge_val(i));

	/* Extra hand-written cases straddling every branch boundary. */
	{
		static const long double extra[] = {
			...
			nextafterl(0.25L, 0.0L),
			nextafterl(0.25L, 1.0L),
			nextafterl(0.5L, 0.0L),
			nextafterl(0.5L, 1.0L),
			nextafterl(0.75L, 0.0L),
			nextafterl(0.75L, 1.0L),
			nextafterl(1.0L, 0.0L),
			nextafterl(1.0L, 2.0L),
			nextafterl(0x1p112L, 0.0L),
			nextafterl(0x1p112L, (long double)INFINITY),
			0x1p112L - 1.0L,
			0x1p112L + 1.0L,
			0x1p113L - 1.0L,
			0x1p113L,
	...
	/* Integer + exact quarter fractions: exercises ar == 0 and fmodl parity. */
	for (i = 0; i < 256; i++) {
		case_sinpil((long double)i);
		case_sinpil((long double)i + 0.25L);
		case_sinpil((long double)i + 0.5L);
		case_sinpil((long double)i + 0.75L);
		case_sinpil(-((long double)i + 0.25L));
	}

	rng_seed(0x0088b008802ULL);
	...
	std::printf("%-20s %12lld %10lld   %s\n", st_sinpil.name,
	    st_sinpil.cases, st_sinpil.fails,
	    st_sinpil.fails == 0 ? "ok" : "FAILED");
	std::printf("------------------------------------------------------"
	    "-\n");
	std::printf("%-20s %12lld %10lld   %s\n", "TOTAL",
	    st_sinpil.cases, st_sinpil.fails,
