// Differential test for PBSD batch b0079s4.
//
// copysignf is compared against the ref_ oracle built from the unmodified
// HardenedBSD source.  Results are compared as raw bit patterns so that NaN
// payloads and the sign of zero are significant.

import pbsd.lib.msun.src.b0079s4;

#include <bit>
#include <cstdint>
#include <cstdio>

namespace ns = pbsd::lib_msun_src::b0079s4;

extern "C" float ref_copysignf(float x, float y);
...
int main(void)
{
	...
	for (unsigned bit = 0; bit < 31; bit++) {
		std::uint32_t mag = 1u << bit;
		check_copysignf(mag | 0x80000000u, 0u);
		check_copysignf(mag | 0x80000000u, 0x80000000u);
		check_copysignf(mag, 0x80000000u);
		check_copysignf(mag, 0u);
	}
	check_copysignf(0x7fffffffu, 0x80000000u);
	check_copysignf(0x80000000u, 0x7fffffffu);
	check_copysignf(0xffffffffu, 0x00000000u);
	check_copysignf(0x00000000u, 0xffffffffu);

	const long long kIters = 260000;
	for (long long i = 0; i < kIters; i++)
		check_copysignf(rand_bits(), rand_bits());

	Stat *all[] = { &st_copysignf };
	const unsigned nall = 1;
