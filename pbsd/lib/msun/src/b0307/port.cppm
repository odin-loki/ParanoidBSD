...
export module pbsd.lib.msun.src.b0307;
...
static inline int irintf(float x)
{
	int n;
...
static inline int irintd(double x)
{
	int n;
	__asm("fistl %0" : "=m"(n) : "t"(x));
	return n;
...
static inline int irintl(long double x)
{
	int n;
	__asm("fistl %0" : "=m"(n) : "t"(x));
	return n;
...
namespace pbsd::lib_msun_src::b0307::detail {
 ...
 * use detail::__kernel_rem_pio2() for large x
 */
...
__ieee754_rem_pio2f(float x, double *y)
{
	...
	SET_FLOAT_WORD(z, ix - ((std::int32_t)((u_std::int32_t)e0<<23)));
	tx[0] = z;
	n  =  detail::__kernel_rem_pio2(tx,ty,e0,1,0);
