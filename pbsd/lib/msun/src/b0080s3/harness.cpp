// Differential test harness for PBSD batch b0080s3.
//
// cargf is compared bit-for-bit against the unmodified C reference in
...
import pbsd.lib.msun.src.b0080s3;

namespace port = pbsd::lib_msun_src::b0080s3;
...
edge_cases(void)
{
	...
	/*
	 * cargf is atan2f(imag, real): every quadrant, both axes, signed zero
	 * and infinity combinations.
	 */
	{
	...
	/* All four signed-zero combinations at the origin. */
	check_cargf(0x00000000u, 0x00000000u, "origin");
	check_cargf(0x80000000u, 0x00000000u, "origin");
	check_cargf(0x00000000u, 0x80000000u, "origin");
	check_cargf(0x80000000u, 0x80000000u, "origin");

	/* Axis cases: real-only and imag-only, both signed zeroes. */
	for (i = 0; i < NFVEC; i++) {
		check_cargf(fvec[i], 0u, "real-only");
		check_cargf(0u, fvec[i], "imag-only");
		check_cargf(fvec[i], 0x80000000u, "imag-negzero");
		check_cargf(0x80000000u, fvec[i], "real-negzero");
	}

	/* Walk both sides of every float boundary that affects atan2f. */
	...
	for (std::size_t k = 0; k < sizeof kWalk / sizeof kWalk[0]; k++) {
		for (int d = -32; d <= 32; d++) {
			...
			check_cargf(re, im, "walk");
			check_cargf(re, kWalk[k], "walk");
			check_cargf(kWalk[k], im, "walk");
			check_cargf(re, re ^ 0x80000000u, "walk");
		}
	}

	/* Near-axis: imag tiny relative to real and vice versa. */
	for (i = 0; i < NFVEC; i++) {
		check_cargf(fvec[i], 0x00000001u, "near-axis");
		check_cargf(fvec[i], 0x80000001u, "near-axis");
		check_cargf(0x00000001u, fvec[i], "near-axis");
		check_cargf(0x80000001u, fvec[i], "near-axis");
	}

	/* Equal-magnitude quadrant boundaries: |re| == |im|. */
	for (i = 0; i < NFVEC; i++) {
		check_cargf(fvec[i], fvec[i], "diag");
		check_cargf(fvec[i], fvec[i] ^ 0x80000000u, "diag");
		check_cargf(fvec[i] ^ 0x80000000u, fvec[i], "diag");
	}
}
...
main(void)
{
	...
	std::printf("pbsd batch b0080s3 differential test\n\n");
	...
	fails = st_cargf.fails;
	std::printf("\n%s: %llu total failures\n",
	    fails == 0 ? "PASS" : "FAIL", fails);

	if (st_cargf.cases == 0) {
		std::printf("cargf: no cases were run\n");
		return 1;
	}
