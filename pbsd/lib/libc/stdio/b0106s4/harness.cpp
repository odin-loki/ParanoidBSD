/*
 * harness.cpp -- differential test for PBSD batch b0106s4 (scanf.c).
 */
...
import pbsd.lib.libc.stdio.b0106s4;

namespace port = pbsd::lib_libc_stdio::b0106s4;
...
std::uint64_t rng_state = 0xb01064deadbeefULL;
...
run_scanf_edges(StatId which)
{
	...
	static const unsigned char nulheavy[] = { '\0', '3', '\0', '7', '\n' };
	static const unsigned char highbit[] = { 0xfe, 0x80, '5', '\n' };
	...
	test_scanf_int(which, "d nulheavy", nulheavy, sizeof(nulheavy), "%d");
	test_scanf_char(which, "c highbit", highbit, sizeof(highbit), "%c");
	if (which == S_SCANF_L)
...
run_scanf_random(StatId which)
{
	...
	for (unsigned i = 0; i < RAND_ITERS; i++) {
		...
		case 5:
			std::strcpy(fmt, "%s");
			test_scanf_str(which, label, input, n, fmt);
			break;
		default:
			std::strcpy(fmt, "%2d");
			test_scanf_int(which, label, input, n, fmt);
