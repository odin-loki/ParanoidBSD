#!/bin/sh
#
# Build and run the b0100 differential test.  Run as `sh build.sh' from
# pbsd/lib/libc/stdio/b0100/.  The harness's exit status is this script's exit
# status.
...
CC=${CC:-cc}
CXX=${CXX:-c++}
CFLAGS=${CFLAGS:--O2 -D_GNU_SOURCE -D_XOPEN_SOURCE=700}
CXXFLAGS=${CXXFLAGS:--O2 -D_GNU_SOURCE -D_XOPEN_SOURCE=700}

MODNAME=pbsd.lib.libc.stdio.b0100
