#!/bin/sh
#
# Build and run the b0143 differential test.
#
# Run as `sh build.sh' from pbsd/lib/libc/stdio/b0143/.  The harness is
# exec'd, so its exit status is this script's exit status.
...
MODNAME=pbsd.lib.libc.stdio.b0143
