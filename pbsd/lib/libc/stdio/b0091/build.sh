#!/bin/sh
#
# Build and run the b0091 differential test.  Run as `sh build.sh' from
# pbsd/lib/libc/stdio/b0091/.  The harness's exit status is this script's
# exit status.
...
MODNAME=pbsd.lib.libc.stdio.b0091
...
if $CXX --version 2>&1 | grep -qi clang; then
	...
	# gcc: -fmodules-ts, the interface first so gcm.cache exists
