#!/bin/sh
# PBSD batch b0012s4: build the oracle, the port and the differential harness,
# link them together and run the harness so its exit status is ours.
#
# Usage: sh build.sh   (from pbsd/lib/libc/softfloat/b0012s4/)

set -e

CC=${CC:-cc}
CXX=${CXX:-c++}
MODULE_NAME=pbsd.lib.libc.softfloat.b0012s4
