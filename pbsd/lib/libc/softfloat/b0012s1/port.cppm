// PBSD -- C++23 port of HardenedBSD lib/libc/softfloat, batch b0012s1.
//
// Ported source:
//   hbsd/src/lib/libc/softfloat/gedf2.c
//
 ...
 * float64_le() primitive that gedf2.c calls are reproduced here from the
 * same package, since gedf2.c cannot be exercised without them:
 *
...
export module pbsd.lib.libc.softfloat.b0012s1;

#define LIT64(a) a##ULL

export namespace pbsd::lib_libc_softfloat::b0012s1 {

/*
-------------------------------------------------------------------------------
Integer types, from the SoftFloat processor milieu.  `flag' is char, so a
negative return value from gedf2() is a negative char, not an int.
-------------------------------------------------------------------------------
*/
typedef char flag;
typedef int int16;
...
/*
-------------------------------------------------------------------------------
gedf2() is __gedf2() of gedf2.c; the leading underscores are dropped because
they are reserved in C++.  Behaviour is unchanged: one less than the reversed
less-or-equal test, so 0 when a >= b and -1 otherwise, including when either
operand is a NaN.
-------------------------------------------------------------------------------
*/
flag gedf2( float64 a, float64 b )
{
...
} // namespace pbsd::lib_libc_softfloat::b0012s1
