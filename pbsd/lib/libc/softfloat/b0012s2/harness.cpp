/*
 * harness.cpp -- differential test for PBSD batch b0012s2.
 *
 * Compares the C++23 port in port.cppm against the unmodified C reference in
 * oracle.c for every function in the batch and every primitive it is built
 * out of:
 *
 *   ledf2               (== __ledf2 of ledf2.c, the batch function)
 *   float64_le          (the SoftFloat primitive __ledf2 is built out of)
 *   extractFloat64Frac  (accessors float64_le is built out of)
 *   extractFloat64Exp
 *   extractFloat64Sign
 *
 * No function writes through a pointer, so the "two guard buffers" protocol
 * is applied to the *result*: both observations are recorded into their own
 * 32-byte buffer pre-filled with the guard byte 0x7f, the result is written
 * at a fixed offset, and the ENTIRE buffer is compared afterwards, including
 * the bytes past the result.  That catches a result whose width, padding or
 * high garbage differs between port and reference, not merely a differing
 * value.  The accumulated float_exception_flags word is compared too, so the
 * NaN path is observable beyond its return value.
 *
 ...
 * (infinity vs NaN), fraction != 0 at an ordinary exponent (which separates
 * the && from a || in the NaN test), exactly one NaN operand in each
 * position with a fall-through answer that differs from the NaN answer
 * (which separates the || from a && in the NaN test), equal signs vs
 * differing signs, a == b vs a one bit either side of b, and the +0/-0 pair
 * that makes the (a|b)<<1 == 0 term decide the answer.
 */
...
import pbsd.lib.libc.softfloat.b0012s2;

namespace port = pbsd::lib_libc_softfloat::b0012s2;

extern "C" {
char ref___ledf2(unsigned long long a, unsigned long long b);
char float64_le(unsigned long long a, unsigned long long b);
unsigned long long extractFloat64Frac(unsigned long long a);
int extractFloat64Exp(unsigned long long a);
char extractFloat64Sign(unsigned long long a);
...
/*
 * Record one observation: guard buffer, exception accumulator zeroed, the
 * raw bytes of the result deposited at SLOT.
 */
template <typename T>
static void
record(Obs &o, T r, unsigned int flags)
{
...
static Obs
port_ledf2(u64 a, u64 b)
{
	Obs o;
	memset(o.buf, 0x7f, sizeof o.buf);
	port::float_exception_flags = 0;
	port::flag r = port::ledf2(a, b);
	record(o, r, (unsigned int)port::float_exception_flags);
	return o;
}

static Obs
ref_ledf2(u64 a, u64 b)
{
	Obs o;
	memset(o.buf, 0x7f, sizeof o.buf);
	float_exception_flags = 0;
	char r = ref___ledf2(a, b);
	record(o, r, float_exception_flags);
	return o;
}
...
static Obs
port_frac(u64 a, u64 b)
{
	Obs o;
	(void)b;
	memset(o.buf, 0x7f, sizeof o.buf);
	port::float_exception_flags = 0;
	port::bits64 r = port::extractFloat64Frac(a);
	record(o, r, (unsigned int)port::float_exception_flags);
	return o;
}

static Obs
ref_frac(u64 a, u64 b)
{
	Obs o;
	(void)b;
	memset(o.buf, 0x7f, sizeof o.buf);
	float_exception_flags = 0;
	unsigned long long r = extractFloat64Frac(a);
	record(o, r, float_exception_flags);
	return o;
}

static Obs
port_exp(u64 a, u64 b)
{
	Obs o;
	(void)b;
	memset(o.buf, 0x7f, sizeof o.buf);
	port::float_exception_flags = 0;
	port::int16 r = port::extractFloat64Exp(a);
	record(o, r, (unsigned int)port::float_exception_flags);
	return o;
}
...
ref_exp(u64 a, u64 b)
{
	...
	memset(o.buf, 0x7f, sizeof o.buf);
	float_exception_flags = 0;
	int r = extractFloat64Exp(a);
	record(o, r, float_exception_flags);
	return o;
}

static Obs
port_sign(u64 a, u64 b)
{
	Obs o;
	(void)b;
	memset(o.buf, 0x7f, sizeof o.buf);
	port::float_exception_flags = 0;
	port::flag r = port::extractFloat64Sign(a);
	record(o, r, (unsigned int)port::float_exception_flags);
	return o;
}

static Obs
ref_sign(u64 a, u64 b)
{
	Obs o;
	(void)b;
	memset(o.buf, 0x7f, sizeof o.buf);
	float_exception_flags = 0;
	char r = extractFloat64Sign(a);
	record(o, r, float_exception_flags);
	return o;
}
...
static Fn fns[] = {
	{ "ledf2 (__ledf2)",    port_ledf2, ref_ledf2, 0, 0, 0 },
	{ "float64_le",         port_le,    ref_le,    0, 0, 0 },
	{ "extractFloat64Frac", port_frac,  ref_frac,  0, 0, 0 },
	{ "extractFloat64Exp",  port_exp,   ref_exp,   0, 0, 0 },
	{ "extractFloat64Sign", port_sign,  ref_sign,  0, 0, 0 },
};
...
run_case(u64 a, u64 b, const char *tag)
{
	for (int i = 0; i < NFN; i++) {
		...
		if (p.ret != r.ret || p.flags != r.flags || bufdiff) {
			...
			if (f.reported < 8) {
				f.reported++;
				printf("FAIL %-18s [%s] a=%016llx b=%016llx "
				    "port{ret=%lld flags=0x%x} ref{ret=%lld "
				    "flags=0x%x} buffer=%s\n",
...
static const u64 base_pool[] = {
	...
	0x3FF0000000000001ULL,	/* nextafter(+1, +inf): frac != 0, exp sane */
...
main(void)
{
	...
	run_case(0x0000000000000000ULL, 0xBFF0000000000000ULL, "+0 vs -1");
	run_case(0xBFF0000000000000ULL, 0x0000000000000000ULL, "-1 vs +0");
	...
	run_case(0x3FF0000000000000ULL, 0xFFF8000000000000ULL, "+1,-qNaN");
	run_case(0xBFF0000000000000ULL, 0x7FF8000000000000ULL, "-1,+qNaN");
	...
	run_case(0x3FF0000000000001ULL, 0x4000000000000000ULL, "frac!=0 <= +2");
	run_case(0x4000000000000000ULL, 0x3FF0000000000001ULL, "+2 !<= frac!=0");
	run_case(0xBFF0000000000001ULL, 0xBFF0000000000000ULL, "-frac!=0 pair");
	...
	run_case(0x8000000000000001ULL, 0x8000000000000002ULL, "-denorm 1,2");
