...
int ref___ssp_real(ppoll)(struct pollfd pfd[], nfds_t nfds,
    ...
    struct __wrusage *ru, siginfo_t *infop, long long ret)
{
	...
	mock_reset(ret);
	rb = port::wait6(idtype, id, psb, options, rub, ib);
	snap_b = take_snap();
    ...
    const sigset_t *newsigmask, long long ret)
{
	...
	install_mocks(ref___libc_interposing);
	mock_reset(ret);
	ra = ref___ssp_real(ppoll)(pa, nfds, ta, ma);
	snap_a = take_snap();

	install_mocks(port::__libc_interposing);
	mock_reset(ret);
	rb = port::__ssp_real(ppoll)(pb, nfds, tb, mb);
	snap_b = take_snap();
...
test_wait6(void)
{
	static const idtype_t idtypes[] = {
		P_PID, P_PPID, P_PGID, P_ALL, P_LWPID, P_JAILID, P_PSETID,
	...
	static const int optss[] = {
		0, WNOHANG, WUNTRACED, WEXITED, WCONTINUED, INT_MIN, INT_MAX,
		0x7f, 0x80,
	};
