#!/bin/sh
#
# Build and run the b0013 differential test.  Run as `sh build.sh' from
# pbsd/lib/libc/gen/b0013/.  The exit status is the harness's exit status.
...
MODNAME=pbsd.lib.libc.gen.b0013
...
if $CXX --version 2>&1 | grep -qi clang; then
	# clang: precompile the interface, then compile the BMI to an object.
	...
	# gcc: -fmodules-ts, and .cppm needs an explicit language.
