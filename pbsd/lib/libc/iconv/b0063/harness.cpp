 ...
 * the ref_ oracle.  Instrumented __bsd_* mocks record every forwarded argument
 * and drive observable return values and side effects.
 */
...
/* ------------------------------------------------------------------ */
/* instrumented __bsd_* mocks (linked by oracle.c and port.cppm)      */
/* ------------------------------------------------------------------ */
...
static const char *
mock_canonicalize(const char *a)
{

	snap.ncalls++;
	snap.canonicalize_arg = a;
	if (a == nullptr) {
		snap.canonicalize_ret = nullptr;
		return (nullptr);
	}

	static char out[CANON_BUF];
	std::size_t i;

	for (i = 0; a[i] != '\0' && i + 1 < CANON_BUF; i++) {
	...
	out[i] = '\0';
	snap.canonicalize_ret = out;
	return (out);
}

static int
mock_get_list(char ***a, size_t *b, __iconv_bool c)
{
	...
	if (a != nullptr) {
		...
		list[2] = nullptr;
		*a = list;
		snap.get_list_out = list;
	}
	if (b != nullptr) {
		*b = c ? 2u : 0u;
		snap.get_list_count = *b;
	}

	snap.get_list_ret = c ? 0 : -1;
...
static void
mock_set_relocation_prefix(const char *a, const char *b)
{
...
__bsd_iconv_set_relocation_prefix(const char *a, const char *b)
{
	...
	mock_set_relocation_prefix(a, b);
}
...
static unsigned long ncase[F_COUNT];
static unsigned long nfail[F_COUNT];
static unsigned nprinted[F_COUNT];
...
static std::uint64_t rng = 0x0063b00630063006ULL;
...
static void
check_canonicalize(const char *label, const char *arg)
{
...
test_canonicalize_pool(unsigned char *pool, std::size_t off)
{
	...
	check_canonicalize("pool", (const char *)(pool + off));
}
...
check_get_list(__iconv_bool c, char ***a_in, size_t *b_in)
{
	...
	if (a_r != nullptr && *a_r != nullptr && *a_p != nullptr) {
		if ((*a_r)[0] != (*a_p)[0] || (*a_r)[1] != (*a_p)[1])
			report_fail(F_GET_LIST, "list pointer mismatch");
	}
	if (b_r != nullptr && b_p != nullptr && *b_r != *b_p)
...
run_edge_cases(void)
{
	...
	check_canonicalize("null", nullptr);
...
run_random_sweep(unsigned long iterations)
{
	...
	for (i = 0; i < iterations; i++) {
		...
		case 0:
			if ((rng_next() & 0xfu) == 0u)
				check_canonicalize("rnd", nullptr);
			else
