...
static void
ref_nul_node_free(void *node __unused)
{
}

/* Find the leftmost node. */
static posix_tnode *
ref_tdestroy_find_leftmost(posix_tnode *tn)
{
	while (tn->llink != NULL)
		tn = tn->llink;
	return (tn);
 ...
 */
void
ref_tdestroy(void *rootp, void (*node_free)(void *))
{
	...
	while (tn != NULL) {
		 ...
		 */
		tn_leftmost = ref_tdestroy_find_leftmost(tn_leftmost);
		if (tn->rlink != NULL) {
