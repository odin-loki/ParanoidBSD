#!/bin/sh
#
# Build and run the b0066s2 differential test.  Run as `sh build.sh' from
# pbsd/lib/libc/gdtoa/b0066s2/.  The harness's exit status is this script's
# exit status.
...
MODNAME=pbsd.lib.libc.gdtoa.b0066s2
...
if $CXX --version 2>&1 | grep -qi clang; then
	...
	$CXX -std=c++23 -fmodules-ts $CXXFLAGS \
	    -c -x c++ port.cppm -o port.o
	$CXX -std=c++23 -fmodules-ts $CXXFLAGS \
	    -c harness.cpp -o harness.o
fi

$CXX -std=c++23 $CXXFLAGS -o harness harness.o port.o oracle.o -lm
