#!/bin/sh
#
# Build and run the b0066s1 differential test.  Run as `sh build.sh' from
# pbsd/lib/libc/gdtoa/b0066s1/.  The harness's exit status is this script's exit
# status.
...
MODNAME=pbsd.lib.libc.gdtoa.b0066s1
ROOT=$(cd ../../../../.. && pwd)
HBSD=$ROOT/hbsd/src
GDTOA=$HBSD/contrib/gdtoa

rm -rf gcm.cache
rm -f oracle.o port.o harness.o port.pcm harness gdtoa_shim.h gdtoa_glue.c gdtoa_glue.o gdtoa_*.o

cat > gdtoa_shim.h <<'EOF'
...
GCFLAGS="-std=c11 $CFLAGS -fno-strict-aliasing -include gdtoa_shim.h -I$GDTOA"
GDTOA_SRCS="dmisc.c misc.c smisc.c sum.c ulp.c hd_init.c hexnan.c gethex.c gmisc.c dtoa.c gdtoa.c"
GDTOA_OBJS=""

$CC $GCFLAGS -c gdtoa_glue.c -o gdtoa_glue.o

for src in $GDTOA_SRCS; do
	obj=gdtoa_${src%.c}.o
	$CC $GCFLAGS -c "$GDTOA/$src" -o "$obj"
	GDTOA_OBJS="$GDTOA_OBJS $obj"
done

$CC -std=c11 $CFLAGS -include gdtoa_shim.h -I$GDTOA \
...
$CXX -std=c++23 $CXXFLAGS -o harness harness.o port.o oracle.o gdtoa_glue.o \
    $GDTOA_OBJS -lm -pthread
