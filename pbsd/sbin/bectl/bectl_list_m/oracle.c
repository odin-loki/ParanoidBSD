...
static const char *ref_get_origin_props(nvlist_t *dsprops, nvlist_t **originprops);
static void ref_print_padding(const char *fval, int colsz, struct printc *pc);
static int ref_print_snapshots(const char *dsname, struct printc *pc);
static void ref_print_info(const char *name, nvlist_t *dsprops, struct printc *pc);
static void ref_print_headers(nvlist_t *props, struct printc *pc);
static unsigned long long ref_dataset_space(const char *oname);
 ...
 */
static const char *
ref_get_origin_props(nvlist_t *dsprops, nvlist_t **originprops)
{
	const char *propstr;
...
static void
ref_print_padding(const char *fval, int colsz, struct printc *pc)
{
...
ref_dataset_space(const char *oname)
{
...
static int
ref_print_snapshots(const char *dsname, struct printc *pc)
{
	nvpair_t *cur;
	...
	for (cur = nvlist_next_nvpair(props, NULL); cur != NULL;
		...
		ref_print_info(nvpair_name(cur), sprops, pc);
	}
	return (0);
}

static void
ref_print_info(const char *name, nvlist_t *dsprops, struct printc *pc)
{
#define	BUFSZ	64
	char buf[BUFSZ];
	...
	if (pc->current_indent == 0 && (pc->show_all_datasets ||
		...
		ref_print_info(dsname, dsprops, pc);
		pc->current_indent += INDENT_INCREMENT;
		if ((oname = ref_get_origin_props(dsprops, &originprops)) != NULL) {
			ref_print_info(oname, originprops, pc);
		...
		pc->current_indent -= INDENT_INCREMENT;
		if (pc->show_snaps)
			ref_print_snapshots(dsname, pc);
		pc->current_indent = 0;
		return;
	} else
		ref_print_padding(name, pc->be_colsz - pc->current_indent, pc);
	...
	ref_print_padding(NULL, active_colsz, pc);
	if (nvlist_lookup_string(dsprops, "mounted", &propstr) == 0) {
		printf("%s", propstr);
		ref_print_padding(propstr, pc->mount_colsz, pc);
	} else {
		printf("%s", "-");
		ref_print_padding("-", pc->mount_colsz, pc);
	}

	oname = ref_get_origin_props(dsprops, &originprops);
	if (nvlist_lookup_string(dsprops, "used", &propstr) == 0) {
		/*
		...
		if (pc->show_space && oname != NULL)
			space += ref_dataset_space(oname);

		/* Alas, there's more to it,. */
		be_nicenum(space, buf, 6);
		printf("%s", buf);
		ref_print_padding(buf, pc->space_colsz, pc);
	} else {
		printf("-");
		ref_print_padding("-", pc->space_colsz, pc);
	}
...
static void
ref_print_headers(nvlist_t *props, struct printc *pc)
{
 ...
 */
static int
ref_prop_list_sort(nvlist_t *props, char *property, bool reverse)
{
	nvpair_t *nvp;
	nvlist_t *nvl;
	int i, nvp_count;
...
int
ref_bectl_cmd_list(int argc, char *argv[])
{
	struct printc pc;
	nvpair_t *cur;
	...
	ref_prop_list_sort(props, column, reverse);

	/* Force -D off if either -a or -s are specified */
	if (pc.show_all_datasets || pc.show_snaps)
		pc.show_space = false;
	if (!pc.script_fmt)
		ref_print_headers(props, &pc);
	...
	for (cur = nvlist_next_nvpair(props, NULL); cur != NULL;
		...
		ref_print_info(nvpair_name(cur), dsprops, &pc);
		printed++;
	}
