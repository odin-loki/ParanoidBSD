...
char *
ref_kvatoname(ipfunc_t func, ioctlfunc_t iocfunc)
{
...
void
ref_ipf_perror(int err, char *string)
{
	if (err == 0)
		fprintf(stderr, "%s\n", string);
	else
...
int
ref_ipf_perror_fd( int fd, ioctlfunc_t iocfunc, char *string)
{
	int save;
	int realerr;
	...
	ref_ipf_perror(realerr, string);
	return (realerr ? realerr : save);

}

void
ref_ipferror(int fd, char *msg)
{
	if (fd >= 0) {
		ref_ipf_perror_fd(fd, ioctl, msg);
	} else {
		fprintf(stderr, "0:");
		perror(msg);
	}
