...
void	ref_usage(char *);
void	ref_printsynchdr(synchdr_t *);
void	ref_printtable(int);
void	ref_printsmcproto(char *);
void	ref_printcommand(int);
int	ref_do_kbuff(int, char *, int *);
int	ref_do_packet(int, char *);
int	ref_buildsocket(char *, struct sockaddr_in *);
void	ref_do_io(void);
void	ref_handleterm(int);
	...
	char *argv[];
{
	...
	if (!debuglevel) {
		...
		setsid();
       	...
       	signal(SIGHUP, ref_handleterm);
       	signal(SIGINT, ref_handleterm);
       	signal(SIGTERM, ref_handleterm);
	...
	while (!terminate) {
		...
		if (ref_buildsocket(interface, &sin) == -1)
			goto tryagain;
...
ref_do_io()
{
	...
	while (!terminate) {
		...
		if (FD_ISSET(lfd, &rd)) {
			...
			switch (ref_do_kbuff(n1, buff, &left))
			{
			case R_IO_ERROR :
				return;
			case R_MORE :
	...
	char *buff;
{
	...
	while (pklen > 0) {
		...
		if (debuglevel > 3) {
			ref_printsynchdr(sh);
			ref_printcommand(sh->sm_cmd);
			ref_printtable(sh->sm_table);
			ref_printsmcproto(buff);
		}
