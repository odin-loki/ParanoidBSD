#!/bin/sh
# Build and run the PBSD b0198 differential test.
#   sh build.sh
...
if "$CXX" --version 2>&1 | grep -qi clang; then
	# Clang: precompile the interface unit, then reuse the BMI.
	...
	# GCC: -fmodules-ts, with the BMI dropped into ./gcm.cache.
	"$CXX" $CXXFLAGS -fmodules-ts -x c++ -c port.cppm -o port.o
	"$CXX" $CXXFLAGS -fmodules-ts -c harness.cpp -o harness.o
	"$CXX" $CXXFLAGS -fmodules-ts -o harness port.o harness.o oracle.o
fi
