/*
 * harness.cpp -- differential test for PBSD batch b0190 (vary.c, strftime_ns).
 */
...
import pbsd.bin.date.b0190;

namespace P = pbsd::bin_date::b0190;
...
size_t ref_strftime_ns(char *, size_t, const char *, const struct tm *, long,
    long);
}
...
test_trans()
{
	...
	auto run = [&](const struct trans *tbl,
	    const decltype(P::trans_mon) ptbl, const char *arg) {
		st.cases++;
	...
	run(trans_mon, P::trans_mon, "january");
	run(trans_mon, P::trans_mon, "jan");
	run(trans_mon, P::trans_mon, "JaN");
	run(trans_mon, P::trans_mon, "february");
	run(trans_mon, P::trans_mon, "feb");
	run(trans_mon, P::trans_mon, "bogus");
	run(trans_mon, P::trans_mon, "");
	run(trans_mon, P::trans_mon, "a");
	run(trans_wday, P::trans_wday, "sunday");
	run(trans_wday, P::trans_wday, "sun");
	run(trans_wday, P::trans_wday, "monday");
	run(trans_wday, P::trans_wday, "MON");
	run(trans_wday, P::trans_wday, "xyz");
	run(trans_wday, P::trans_wday, "\xff\xff\xff");
	...
	for (long i = 0; i < SWEEP / 14; i++) {
		...
		if (rng.coin())
			run(trans_mon, P::trans_mon, buf);
		else
			run(trans_wday, P::trans_wday, buf);
	}
}
