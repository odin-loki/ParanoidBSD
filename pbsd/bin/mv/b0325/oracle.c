/*
 * PBSD batch b0325 oracle.
 *
 * hbsd/src/bin/mv/mv.c: every function renamed with a "ref_" prefix;
 * the "static" storage class is dropped on file-scope functions so the
 * differential harness can link against them.  Function bodies are
...
int	fflg, hflg, iflg, nflg, vflg;

int	ref_copy(const char *, const char *);
int	ref_do_move(const char *, const char *);
int	ref_fastcopy(const char *, const char *, struct stat *);
void	ref_usage(void);
...
ref_main(int argc, char *argv[])
{
	...
	if (stat(argv[argc - 1], &sb) || !S_ISDIR(sb.st_mode)) {
		...
		exit(ref_do_move(argv[0], argv[1]));
	}
...
int
ref_do_move(const char *from, const char *to)
{
...
#define YESNO "(y/n [n]) "
	 ...
	 * with EXDEV.  Therefore, ref_copy() doesn't have to perform the checks
	...
	return (S_ISREG(sb.st_mode) ?
	    ref_fastcopy(from, to, &sb) : ref_copy(from, to));
}

int
ref_fastcopy(const char *from, const char *to, struct stat *sbp)
{
...
err:		if (unlink(to))
	 ...
	 * source_file.
	 */
	ref_preserve_fd_acls(from_fd, to_fd, from, to);
	(void)close(from_fd);
...
int
ref_copy(const char *from, const char *to)
{
	struct stat sb;
	int pid, status;
...
void
ref_preserve_fd_acls(int source_fd, int dest_fd, const char *source_path,
