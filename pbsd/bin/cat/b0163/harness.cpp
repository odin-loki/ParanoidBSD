...
constexpr std::size_t RAND_MAX_LEN = 16384;
...
bool
same_raw(const IoResult &a, const IoResult &b)
{
...
bool
read_file(int fd, std::vector<unsigned char> *out)
{
	unsigned char buf[8192];
	ssize_t nr;
    ...
    std::size_t len)
{
	...
	char refoutpath[] = "/tmp/pbsd_b0163_orefXXXXXX";
	char portoutpath[] = "/tmp/pbsd_b0163_oportXXXXXX";
	int infd;
	int refoutfd;
	int portoutfd;
	pid_t pid;
	int refstatus = 0;
	int portstatus = 0;
	std::vector<unsigned char> refdata;
	std::vector<unsigned char> portdata;

	st.cases++;
	infd = mkstemp(inpath);
	refoutfd = mkstemp(refoutpath);
	portoutfd = mkstemp(portoutpath);
	if (infd < 0 || refoutfd < 0 || portoutfd < 0) {
		st.fails++;
		if (infd >= 0) {
			close(infd);
			unlink(inpath);
	...
	close(portoutfd);
	waitpid(pid, &portstatus, 0);

	refoutfd = open(refoutpath, O_RDONLY);
	portoutfd = open(portoutpath, O_RDONLY);
	if (refoutfd < 0 || portoutfd < 0 ||
	    !read_file(refoutfd, &refdata) ||
	    !read_file(portoutfd, &portdata)) {
		st.fails++;
	} else if (!WIFEXITED(refstatus) || WEXITSTATUS(refstatus) != 0 ||
	    !WIFEXITED(portstatus) || WEXITSTATUS(portstatus) != 0 ||
	    refdata != portdata) {
		st.fails++;
	}
	if (refoutfd >= 0)
...
test_raw_cat()
{
	...
	test_raw_cat_case(st, empty, 0);
	test_raw_cat_case(st, one, 1);
	test_raw_cat_case(st, nul, sizeof(nul));
	test_raw_cat_case(st, high, sizeof(high));
	for (std::size_t sz : sizes)
		test_raw_cat_case(st, block, sz);
	...
	for (long i = 0; i < RANDOM_ITERS; i++) {
		unsigned char buf[RAND_MAX_LEN];
		std::size_t len = (std::size_t)(rnd() % (RAND_MAX_LEN + 1));

		for (std::size_t j = 0; j < len; j++)
			buf[j] = rnd_byte();
		test_raw_cat_case(st, buf, len);
	}
}
...
bool
copy_files_open(CopyFiles *cf)
{
	char inpath[] = "/tmp/pbsd_b0163_kinXXXXXX";
	char outpath[] = "/tmp/pbsd_b0163_koutXXXXXX";

	cf->infd = mkstemp(inpath);
	cf->outfd = mkstemp(outpath);
	if (cf->infd < 0 || cf->outfd < 0) {
		if (cf->infd >= 0) {
			close(cf->infd);
			unlink(inpath);
		}
...
IoResult
run_ref_in_kernel_copy_on_stdout(int rfd)
{
	IoResult res{};
...
IoResult
run_port_in_kernel_copy_on_stdout(int rfd)
{
	IoResult res{};
...
void
test_in_kernel_copy_case(Stat &st, CopyFiles *cf, const unsigned char *data,
    std::size_t len)
{
	int saved_stdout;
	IoResult ref_res;
	...
	st.cases++;
	if (ftruncate(cf->infd, 0) != 0 || lseek(cf->infd, 0, SEEK_SET) < 0) {
		st.fails++;
		return;
	}
	if (len > 0 && write(cf->infd, data, len) != (ssize_t)len) {
		st.fails++;
		return;
	}
	if (lseek(cf->infd, 0, SEEK_SET) < 0) {
		st.fails++;
		return;
	}
	if (ftruncate(cf->outfd, 0) != 0 || lseek(cf->outfd, 0, SEEK_SET) < 0) {
		st.fails++;
		return;
	}
	if (save_stdout(&saved_stdout) < 0) {
	...
	dup2(cf->outfd, STDOUT_FILENO);
	ref_res = run_ref_in_kernel_copy_on_stdout(cf->infd);
	fflush(stdout);
	if (lseek(cf->outfd, 0, SEEK_SET) < 0 ||
	    !read_file(cf->outfd, &ref_res.out)) {
		restore_stdout(saved_stdout);
		st.fails++;
		return;
	}
	...
	dup2(cf->outfd, STDOUT_FILENO);
	port_res = run_port_in_kernel_copy_on_stdout(cf->infd);
	fflush(stdout);
	if (lseek(cf->outfd, 0, SEEK_SET) < 0 ||
	    !read_file(cf->outfd, &port_res.out)) {
		restore_stdout(saved_stdout);
		st.fails++;
		return;
	}
...
void
test_in_kernel_copy_invalid_fd(Stat &st, CopyFiles *cf)
{
	int saved_stdout;
	IoResult ref_res;
	IoResult port_res;
	...
	dup2(cf->outfd, STDOUT_FILENO);
	ref_res = run_ref_in_kernel_copy_on_stdout(-1);
	fflush(stdout);
	if (lseek(cf->outfd, 0, SEEK_SET) < 0 ||
	    !read_file(cf->outfd, &ref_res.out)) {
		restore_stdout(saved_stdout);
	...
	dup2(cf->outfd, STDOUT_FILENO);
	port_res = run_port_in_kernel_copy_on_stdout(-1);
	fflush(stdout);
	if (lseek(cf->outfd, 0, SEEK_SET) < 0 ||
	    !read_file(cf->outfd, &port_res.out)) {
		restore_stdout(saved_stdout);
...
test_in_kernel_copy()
{
	...
	if (!copy_files_open(&cf))
		return;

	for (std::size_t i = 0; i < sizeof(block); i++)
		block[i] = rnd_byte();

	test_in_kernel_copy_case(st, &cf, empty, 0);
	test_in_kernel_copy_case(st, &cf, one, 1);
	test_in_kernel_copy_case(st, &cf, nul, sizeof(nul));
	test_in_kernel_copy_case(st, &cf, high, sizeof(high));
	for (std::size_t sz : sizes)
		test_in_kernel_copy_case(st, &cf, block, sz);
	test_in_kernel_copy_invalid_fd(st, &cf);
	...
	for (long i = 0; i < RANDOM_ITERS; i++) {
		...
		for (std::size_t j = 0; j < len; j++)
			buf[j] = rnd_byte();
		test_in_kernel_copy_case(st, &cf, buf, len);
	}
