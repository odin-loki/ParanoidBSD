...
std::vector<P::statfs_type> g_mnt_table;
...
extern "C" int __wrap_getmntinfo(struct statfs **mntbufp, int mode) {
	(void)mode;
	static std::vector<P::statfs_type> storage;
	storage = g_mnt_table;
...
bool statfs_equal(const P::statfs_type &a, const P::statfs_type &b) { return std::memcmp(&a,&b,sizeof(P::statfs_type))==0; }
bool maxwidths_equal(const P::maxwidths_type &a, const P::maxwidths_type &b) { return std::memcmp(&a,&b,sizeof(P::maxwidths_type))==0; }

P::statfs_type make_statfs(const char *from, const char *on, const char *type,
    int64_t bsize, int64_t blocks, int64_t bfree, int64_t bavail,
    int64_t files, int64_t ffree, uint64_t flags=0) {
	P::statfs_type s{}; s.f_flags=flags; s.f_bsize=bsize; s.f_blocks=blocks;
...
bool test_addstat_case(const P::statfs_type &add,int64_t tbs) {
	st_addstat.cases++;
	P::statfs_type tr{}, tp{}, ar=add, ap=add;
...
bool test_update_maxwidths_case(const P::statfs_type &s) {
	st_update_maxwidths.cases++;
	P::maxwidths_type mr{}, mp{};
...
bool test_regetmntinfo_case(std::vector<P::statfs_type> t,long ms,bool lists) {
	st_regetmntinfo.cases++;
	std::vector<P::statfs_type> br=t,bp=t; P::statfs_type *rp=br.data(), *pp=bp.data();
	if (lists) {
...
bool test_prthuman_case(const P::statfs_type &s,int64_t u,int hf) {
...
bool test_prtstat_case(P::statfs_type s,int hf,int inf,int tf,int th) {
	st_prtstat.cases++;
	P::maxwidths_type mr{}, mp{}; P::statfs_type sr=s, sp=s;
...
void hand_tests() {
	...
	P::statfs_type add=make_statfs("x","/x","ufs",4096,100,20,70,1000,800);
	test_addstat_case(add,512); test_addstat_case(add,4096);
	P::statfs_type s=make_statfs("dev","/mnt","ufs",4096,10000,2000,7000,50000,40000);
	...
	P::statfs_type hs=make_statfs("d","/m","ufs",512,100,10,80,0,0);
	test_prthuman_case(hs,90,1); test_prthuman_case(hs,90,2);
	P::statfs_type ps=make_statfs("dev/da0","/mnt","ufs",512,1000,100,800,10000,9000);
	...
	char *avp[]={prog,path,nullptr}; g_stat_map[path]=struct stat{0}; test_main_case(2,avp);
...
void sweeps() {
	...
	for (long i=0;i<SWEEP;++i) {
		P::statfs_type add=make_statfs("d","/m","ufs",(int64_t)(rng.next()%8192+512),
	...
	for (long i=0;i<SWEEP;++i) {
		P::statfs_type s=make_statfs("d","/m","ufs",(int64_t)(rng.next()%8192+1),
