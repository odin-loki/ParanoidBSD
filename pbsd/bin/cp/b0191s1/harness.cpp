...
{
	for (long i = 0; i < 24937; i++)
		(void)run_copy_fallback_case("sweep");
}

static void test_copy_file_hand()
{
	(void)run_copy_file_case("dne", true, false, 0);
	(void)run_copy_file_case("exists", false, false, 0);
}

static void test_copy_file_sweep()
{
	for (long i = 0; i < 24937; i++)
		(void)run_copy_file_case("sweep", rng.coin(), rng.coin(), rng.bits(0,11));
}
...
static int run_all_tests(){test_copy_fallback_hand();test_copy_file_hand();test_copy_link_hand();test_copy_fifo_hand();test_copy_special_hand();test_setfile_hand();test_preserve_fd_acls_hand();test_preserve_dir_acls_hand();test_usage_hand();test_copy_fallback_sweep();test_copy_file_sweep();test_copy_link_sweep();test_copy_fifo_sweep();test_copy_special_sweep();test_setfile_sweep();test_preserve_fd_acls_sweep();test_preserve_dir_acls_sweep();test_usage_sweep();Stat*all[]={&st_copy_fallback,&st_copy_file,&st_copy_link,&st_copy_fifo,&st_copy_special,&st_setfile,&st_preserve_fd_acls,&st_preserve_dir_acls,&st_usage};long tc=0,tf=0;std::printf("\n%-22s %12s %12s\n","function","cases","failures");for(Stat*st:all){std::printf("%-22s %12ld %12ld\n",st->name,st->cases,st->fails);tc+=st->cases;tf+=st->fails;}std::printf("%-22s %12ld %12ld\n","TOTAL",tc,tf);return tf==0?0:1;}
