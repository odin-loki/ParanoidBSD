#!/bin/sh
#
# Build and run the b0179 differential test.  Run as `sh build.sh' from
# pbsd/bin/chflags/b0179/.  The harness's exit status is this script's exit
# status.
...
MODNAME=pbsd.bin.chflags.b0179

WRAP_LDFLAGS="-Wl,--wrap=chflagsat -Wl,--wrap=fts_open -Wl,--wrap=fts_read \
    -Wl,--wrap=fts_set -Wl,--wrap=fts_close -Wl,--wrap=exit"
...
$CXX -std=c++23 $CXXFLAGS $WRAP_LDFLAGS -o harness harness.o port.o oracle.o
